...
//     Idr_i = (idtFdr.read() - Xm*Ids)/REA_ROTOR;
//     Iqr_i = (idtFqr.read() - Xm*Iqs)/REA_ROTOR;
//     ddtFdr_i = -RES_ROTOR*Idr_i + S*idtFqr.read();
//     ddtFqr_i = -RES_ROTOR*Iqr_i - S*idtFdr.read();
//     Fiqs = REA_STATOR*Iqs + Xm*Iqr_i;
//     Fids = REA_STATOR*Ids + Xm*Idr_i;
//     power = REF_VOLT_DS * (Ids - Iqs) + REF_VOLT_QS * (Ids - Iqs);
...
//     }
		...
		/*
		   ...
		   while(!stop){
			 ...
			 Idr_i_new = (idtFdr.read() - Xm*Ids)/REA_ROTOR;
			 Fids_new = REA_STATOR*Ids_new + Xm*Idr_i_new;
			 Iqs_new = -(Fids_new + 1000)/0.012100000000000;
			 Iqr_i_new = (idtFqr.read() - Xm*Iqs_new)/REA_ROTOR;
			 Fiqs_new = REA_STATOR*Iqs_new + Xm*Iqr_i_new;
