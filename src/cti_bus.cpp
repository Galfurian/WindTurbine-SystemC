...
//	cout << "Total generation " << total_generation << "\n";
    ...
    { // Load is larger than generation
        ...
        if (batt_soc > 0.1)
        { // Battery has energy, can provilde power to load to compensate generation is not enough
        ...
        { // Battery is dead, cannot provide any more; the power need buy from grid;

            Ibatt_cnv.write(0.0); // Battery no action;
            Buy_from_grid.write(
                Ibatt_tmp * VBUS); // Store the value of buy from grid
    ...
    { // Load is less than generation
        ...
        { // The extra power can be absorbed in battery
            ...
            Ibatt_cnv.write(
                Ibatt_tmp); // Battery charge current, it is an negative value
            Sell_to_grid.write(0.0); // Store the value for selling to grid

        }
        else
        { // Battery is fully charged, cannot charge anymore, the extra power can sell to grid

            Ibatt_cnv.write(0.0); // Battery no action
            Sell_to_grid.write(
                Ibatt_tmp * VBUS); // Store the value for selling to grid
